class RamoEventsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @ramo_event.save
        format.html { redirect_to @ramo_event, notice: "Ramo event was successfully created." }
  ...
  def update
    respond_to do |format|
      if @ramo_event.update(ramo_event_params)
        format.html { redirect_to @ramo_event, notice: "Ramo event was successfully updated." }
  ...
  private
    ...
    def ramo_event_params
      params.require(:ramo_event).permit(:type, :day_of_week, :start_timestamp, :end_timestamp, :date)
    end
end
